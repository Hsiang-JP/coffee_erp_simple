...
```markdown
# GREEN COFFEE ERP: "Coffee JOurney" PWA DEMO
**Version:** 1.0.0
...
* **Precedence:** If a global schema is found, it overrides local interpretations of table structures (e.g., specific field names for 'Batch' or 'Producer'). Don't modify the global schema `Database Schema/simple_db.sql` this is the source of truth.
...
* **Unified State Synchronization:** All data fetching MUST be consolidated into a single `syncStore` action in the Zustand store. Components MUST NOT perform direct database queries for general data loading. The database is the single source of truth; the store is the single source of projection. Ensure the `refreshTrigger` is the only mechanism for re-syncing data after mutations.
...
* **Features (Greedy + Weights):**
* *Step 1 (Filter):* Query all bags where `status = 'Available'` and `lot.score >= Min Score`, `variety` and `flavor notes`.
...
Flavor notes are the `reward`, do a fuzzy match, use the match count, for example, for all the possible combinations, one match count add 2 points to the quality score. Discuss with your team for the best way to implement this.

For this part, consult with a optimization experts to find good ways to get to these results.


* **UI Output:** Display 4 recommended bag combinations. Show a breakdown of the *Cost Per KG* for the suggested combination to aid the sales team in pricing. Show the detail bags information under each combination, including public_id, weight (show the split weight if the bag is splitted), stock_code 

* **Inventory visualization:** Use CSS sqaures to show the map of the warehouse. For example, make a 10 by 6 grid, the 10 is the stack height and the 6 is the palette count, each palette will be AA, AB, AC, AD, AE, AF. The default is white, meaning empty. When it's blue means occupied by one bag. Thicken the boarder when the bag is selected for allocation. When the mouse hover over, show the farm, variety, weight. 

Consult with your ui artist to optimze this part.

### Page 4: Data Entry

* **Goal:** To register data to the database while running the business
Make sure the SQL language is robust and bug-free. Now we have three tabs:

1. But the lot
2. Cupping sessions
3. Register producer and farm

### Page dev

* **Goal:** To edit or delete data if needed. 
