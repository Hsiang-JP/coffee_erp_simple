...
**Version:** 1.0.0
**Tech Stack:** React (Vite/PWA), wa-sqlite (Browser WASM Database), React-Simple-Maps (SVG Map), Zustand (State Management), TailwindCSS (Styling).
...
* **Precedence:** If a global schema is found, it overrides local interpretations of table structures (e.g., specific field names for 'Batch' or 'Producer'). Don't modify the global schema `schema.js` this is the source of truth.
...
* **Role:** Build components, css animations, SVG map rendering, and Tailwind styling.
* **Directives:** Keep animations smooth, build responsive cards, and ensure the UI reflects the "Premium Specialty Coffee" aesthetic. You own the visual interpretation of data.
...
* **Directives:** Ensure all database updates are atomic. 
* **DB-first:** Make sure using trigger and view to manage complicated and automatic logic in the backend.
...
* **Goal:** Show the financial and physical journey of a single bag or lot simultaneously. Communivate traceability with clients.
...
* When a stage updates (e.g., Farm -> Cora), make sure the value chain is updated and intact. 


### Page 2: Smart Allocation
...
Use Operational Cost (Prefer Level 3 or 4 over Level 1) to make final decisions, use the sotck_code of the bags table to calculate the cost, for example, code: AA-1, AA is the palette code, and the 1 is the level. 1 means ground level, which is the hardest to fetch if the stack is full. And 10 is the highest level, by definition the easiest to get.

For this part, consult with a optimization experts to find good ways to get to these results.


* **UI Output:** Display 3 recommended bag combinations. Show a breakdown of the *Cost Per KG* for the suggested combination to aid the sales team in pricing. 

* **Inventory visualization:** Use CSS sqaures to show the map of the warehouse. For example, make a 10 by 6 grid, the 10 is the stack height and the 6 is the palette count, each palette will be AA, AB, AC, AD, AE, AF. The default is white, meaning empty. When it's black means occupied by one bag. Show an mark when the bag is selected for allocation. 

Consult with your ui artist to optimze this part.

### Page 3: QC Calibration Reports

* **Goal:** Visualize cupping data to identify lot quality and cupper bias.
* **Filter UI:** 3 simple dropdowns/inputs: `Farm Name`, `Cupper Name`, `Lot`. Logic is strictly **AND**.
...
* **Goal:** Enter data as the coffee industry supply chain
* **Features:** Enter the data to the system. 

### One dev page for data management

* **Goal:** Edit or delete data once it's added. 
* **Features:** Database maintenance.
